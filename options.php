...
<a href="options.php?product=<?php echo $product ?>" class="sideLink"><li class="active-step">Step 2: Options</li></a>
</ul>
</div>
<?php
...
$sql = 'SELECT PrintProduct.printProdId, PrintProduct.printProdName
FROM PrintProduct
...
<p><strong>Enter a name for this project</strong></p>
<form id="project-info" method="POST" action="products.php">
<input type="text" name="project-name" size="40" />
</form>
...
WHERE PrintProduct.printProdId = '.$product.' AND PrintProduct.printProdId = PrintProductStyle.printProdId
';
...
$("#project-info").attr("action", "products.php?style=" + $(".style-thumb:first").attr("id"));
