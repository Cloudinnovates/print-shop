...
$pdf = new FPDF(''.$printProdOrientation.'', 'in', array($printProdWidth + .5, $printProdHeight + .5));
...
$pdf->AddPage();
$pdf->Image($styleBackground, .25, .25, $pdf->w - .5, $pdf->h - .5);
...
foreach ($rows as $row) {
	...
	// God damnit. Now they've got a design they want to use that has borders. 
	// So now nothing fits perfectly in the borders, because no other design uses
	// borders. God damnit. 
	$pdf->SetFont("Impact","",19);
	$pdf->SetTextColor($styleFontHeadR,$styleFontHeadG,$styleFontHeadB);
	if ($styleId == 89 || $styleId == 90) {
		$pdf->SetXY(.40,0.40);	
	}
	$pdf->SetXY(.25,0.25);
